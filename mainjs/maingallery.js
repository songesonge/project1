$(document).ready(function() {
    ...
    $('.btn1').css('background','rgb(255, 183, 147)'); //첫번째 불켜
    $('.btn1').css('width','30'); // 버튼의 너비 증가
    
    $('.gallery .link1').fadeIn('slow'); //첫번째 이미지 보인다..rgb(118, 66, 148)rgb(118, 66, 148)
    ...
    //   }
      ...
      $('.mbutton').css('background','rgb(236, 228, 236)'); //버튼불다꺼!!
      $('.mbutton').css('width','10'); // 버튼 원래의 너비
      $('.btn'+cnt).css('background','rgb(255, 183, 147)');//자신만 불켜
      $('.btn'+cnt).css('width','30');    

       if(cnt==imageCount)cnt=0;  //카운트의 초기화 0
     }
     
    timeonoff= setInterval( moveg , 9000);// 타이머를 동작 1~5이미지를 순서대로 자동 처리
   ...
   $('.mbutton').click(function(event){  //각각의 버튼 클릭시
      ...
      //   $('.btn'+i).css('width','16');
		  // }
      $('.mbutton').css('background','rgb(236, 228, 236)'); //버튼 모두불꺼
      $('.mbutton').css('width','10');
      $('.btn'+cnt).css('background','rgb(255, 183, 147)');//자신 버튼만 불켜 
      $('.btn'+cnt).css('width','30');
      
      if(cnt==imageCount)cnt=0;  //카운트 초기화
     
      timeonoff= setInterval( moveg , 9000); //타이머의 부활!!!
     
      if(onoff==false){ //중지상태냐??
            onoff=true; //동작~~
            $('.ps').css('background','url(images/stop.svg)');
      }
  ...
  $('.ps').click(function(){ 
     if(onoff==true){ // 타이머가 동작 중이냐??
	       clearInterval(timeonoff);   //살인마 고용 stop버튼 클릭시
		     $(this).css('background','url(./images/play.svg)'); //js파일에서는 경로의 기준이 html파일이 기준!!
         onoff=false;   
	   }else{  // false 타이머가 중지 상태냐??
		   timeonoff= setInterval( moveg , 9000); //play버튼 클릭시  부활
		   $(this).css('background','url(./images/stop.svg)');
		   onoff=true; 
	   }
  });	
